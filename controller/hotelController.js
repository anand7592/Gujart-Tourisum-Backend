...
exports.createHotel = async (req, res, next) => {
  try {
    ...
    const {
      name, description, place, location, address, contactNo,
      email, website, pricePerNight, category,
      amenities, roomTypes
    } = req.body;
    ...
    try {
      ...
      return res.status(400).json({ message: "Invalid JSON format for roomTypes" });
    }
    ...
    // 4. Validation
    if (!name || !description || !place || !location || !address || !contactNo || !category) {
      return res.status(400).json({ message: "Please fill in all required fields (Name, Place, Location, Address, Contact, Category)" });
    }
...
exports.updateHotel = async (req, res, next) => {
  try {
    ...
    const fields = ['name', 'description', 'place', 'location', 'address', 'contactNo', 'email', 'website', 'category'];
    fields.forEach(field => {
      if (req.body[field]) hotel[field] = req.body[field];
    });

    if (req.body.pricePerNight) hotel.pricePerNight = Number(req.body.pricePerNight);
    if (req.body.isActive !== undefined) hotel.isActive = req.body.isActive === 'true'; // Handle string boolean
